...
tb1_juv_wind$yDateq <- tb1_juv_wind[, "yDate"]^2

#select for columns of interest
tb2_juv <- select(tb1_juv_wind, Date, Plant_lights, Total_juv_unk_LESP, Fog_Cat, CloudCover_Cat, yDate, yDateq, Lunar, Year, Wind_Speed, OnOffShore) %>%
  drop_na(Date, Plant_lights, Total_juv_unk_LESP, Fog_Cat, CloudCover_Cat, yDateq, Lunar, Year, Wind_Speed, OnOffShore) %>%
...
mod1_tb <- lm(Total_juv_unk_LESP ~ Lunar + yDateq + yDate + Plant_lights + Wind_Speed + OnOffShore + CloudCover_Cat + Year, data = tb2_juv)
...
mod2_tb <- glmmTMB(Total_juv_unk_LESP ~ Lunar + yDateq + yDate + Plant_lights + Wind_Speed + OnOffShore + CloudCover_Cat + Year, data = tb2_juv, family = poisson(link = "log"))
...
mod3_tb <- glmmTMB(Total_juv_unk_LESP ~ Lunar + yDateq + yDate + Plant_lights + Wind_Speed + OnOffShore + Year + CloudCover_Cat, data = tb2_juv, family = nbinom1(link = "log"))
...
plotResiduals(form = tb2_juv$yDateq, simulationOutput = assumptions)
...
#lightsmod4 - negative binomial excluding outlier - poor fit
mod4_tb <- glmmTMB(Total_juv_unk_LESP ~ Lunar + yDateq + Plant_lights + OnOffShore + Year + CloudCover_Cat, data = tb2_juv_outlier, family = nbinom1(link = "log"))
...
plotResiduals(form = tb2_juv_outlier$yDateq, simulationOutput = assumptions)
